...
describe('教師修改補課安排 API', () => {
    ...
    it('教師可修改補課日期與時間', async () => {
        const response = await (0, supertest_1.default)(app_1.app)
            .put(`/api/make_up_sessions/${makeUp.id}`)
            .set('Authorization', `Bearer ${teacherToken}`)
            .send({ make_up_date: '2024-03-15', start_time: '20:00', end_time: '22:00' });
        expect(response.status).toBe(200);
        expect(response.body.success).toBe(true);
        // 驗證資料已更新
        const updated = await database_1.AppDataSource.getRepository(MakeUpSession_1.MakeUpSession).findOne({ where: { id: makeUp.id } });
        expect(updated?.make_up_date).toBe('2024-03-15');
        expect(updated?.start_time).toBe('20:00');
        expect(updated?.end_time).toBe('22:00');
    });
    it('非教師不可修改補課安排', async () => {
        const response = await (0, supertest_1.default)(app_1.app)
            .put(`/api/make_up_sessions/${makeUp.id}`)
    ...
    it('欄位驗證失敗', async () => {
        const response = await (0, supertest_1.default)(app_1.app)
            .put(`/api/make_up_sessions/${makeUp.id}`)
            .set('Authorization', `Bearer ${teacherToken}`)
            .send({ make_up_date: '', start_time: '', end_time: '' });
