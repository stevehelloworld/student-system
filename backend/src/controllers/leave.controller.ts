...
import { MakeUpSession, MakeUpAttendanceStatus } from '../models/MakeUpSession';
...
export const deleteLeave = async (req: Request, res: Response) => {
  try {
    ...
    if (
      !(
        (req.user.role === UserRole.STUDENT && leave.student_id === req.user.id) ||
...
export const putLeaveByTeacher = async (req: Request, res: Response) => {
  try {
...
export const arrangeMakeUpSession = async (req: Request, res: Response) => {
  if (!req.user || req.user.role !== UserRole.TEACHER) {
...
export const getMyMakeUpSessions = async (req: Request, res: Response) => {
...
export const updateMakeUpAttendance = async (req: Request, res: Response) => {
  if (!req.user || req.user.role !== UserRole.TEACHER) {
  ...
  if (!Object.values(MakeUpAttendanceStatus).includes(attendance_status)) {
...
export const updateMakeUpSession = async (req: Request, res: Response) => {
  if (!req.user || req.user.role !== UserRole.TEACHER) {
...
export const deleteMakeUpSession = async (req: Request, res: Response) => {
  if (!req.user || req.user.role !== UserRole.TEACHER) {
